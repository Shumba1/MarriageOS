...
| **1. Repo foundation**                    | Stand up repo skeleton, shell split, routes, theme, and registry                                       | Canonical `routes.ts`, `theme.ts`, `page-data.ts`, component registry guidance, repo structure guidance from code samples            | `src/config/routes.ts`, `src/config/theme.ts`, `src/config/page-data.ts`, `src/config/component-registry.ts`, route groups, `AppShell`, `MarketingShell`, `TopBar`, `BottomNav`, `SideNav`, `HeroBlock`, `Card` | Typecheck, lint, route render check, route-to-component map sanity check, shell responsiveness, safe-area and `100dvh` behaviour                                                    | Does the product already feel app-like rather than like a marketing site with tabs? Are route/component assumptions clean?                    | **Go** only if all configured routes render, registry matches routes, and shell/nav work cleanly on mobile and desktop                                                                                     |
...
| --- | --------------------- | --------------------------- | ----------------------------------------------------------------------- | ----------- | -------- | --------- | -------- | ------ | ------------- |
| 0.1 | Governance            | Canonical layer present     | `MASTER_SPEC.md`, `DRIFT_REGISTER.md`, `theme.ts`, `page-data.ts`       | ☐           | ☐        | ☐         | ☐        | ☐      |               |
